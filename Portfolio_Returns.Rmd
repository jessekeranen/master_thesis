...
data2 <- factor(data2[Date > as.POSIXct("1994-7-31", tz = "UTC") &
                ...
                "FM.EXP.RET", "FM.POSITION", c("FM.Short", "FM.2", "FM.3", "FM.4",
                 "FM.5", "FM.6", "FM.7", "FM.8", "FM.9", "FM.Long"), quantiles)
data2 <- factor(data2, "RF.EXP.RET", "RF.POSITION", c("RF.Short", "RF.2", "RF.3",
            "RF.4", "RF.5", "RF.6", "RF.7", "RF.8", "RF.9", "RF.Long"), quantiles)
data2 <- factor(data2, "NN.EXP.RET", "NN.POSITION", c("NN.Short", "NN.2", "NN.3",
            "NN.4", "NN.5", "NN.6", "NN.7", "NN.8", "NN.9", "NN.Long"), quantiles)
...
plot3 <- ggplot() + geom_line(data = portfolio_returns_long[Position %in% c("Long", "Short") & variable == "VW.RET"], aes(Date, cum_prod, color = Method, linetype = Position)) + scale_y_log10() + geom_line(data = market_return, aes(Date, cum_prod)) +
...
plot4 <- ggplot() + geom_line(data = portfolio_returns_long[Position %in% c("Long", "Short") & variable == "EW.RET"], aes(Date, cum_prod, color = Method, linetype = Position)) + scale_y_log10() + geom_line(data = market_return, aes(Date, cum_prod)) +
  theme(axis.title = element_blank(), legend.position = "bottom", text =
          element_text(size = 9))
print(plot4)
dev.off()
...
plot5 <- ggplot(r2_ts, aes(Date, R2)) + geom_line() + facet_wrap(~temp, ncol = 2) +
  theme(axis.title = element_blank(), text = element_text(size = 9))
print(plot5)
dev.off()
plot5

FM_r2_plot <- ggplot(FM_r2s_scaled[R2.Method == "R2OOS"], aes(y = reorder(method, V1),
                    x = V1)) + geom_bar(stat='identity', fill = "#13bfc4") +
  theme(axis.title = element_blank(), text = element_text(size = 9))
...
plot6 <- grid.arrange(FM_r2_plot + facet_wrap(facets = as.factor("FM R2")) + ggtitle(NULL) + theme(axis.title = element_blank()), trad_FM_r2_plot + facet_wrap(facets = as.factor("FM Trad. R2")) + ggtitle(NULL) + theme(axis.title = element_blank()), RF_r2_plot + facet_wrap(facets = as.factor("RF R2")) + ggtitle(NULL) +  theme(axis.title = element_blank()), trad_RF_r2_plot + facet_wrap(facets = as.factor("RF Trad. R2")) + ggtitle(NULL) +  theme(axis.title = element_blank()), NN_r2_plot + facet_wrap(facets = as.factor("NN R2")) + ggtitle(NULL) + theme(axis.title = element_blank()), trad_NN_r2_plot + facet_wrap(facets = as.factor("NN Trad. R2")) + ggtitle(NULL) + theme(axis.title = element_blank()), nrow = 3)
print(plot6)
dev.off()
...
plot7 <- ggplot(temp5, aes(variable, method)) + geom_tile(aes(fill = value), show.legend = F) +
...
drawdown <- function(dt) {
  ...
  drawdown <- cum_profit[, .(DRAWDOWN = (CUM.RET - cummax(CUM.RET))/ cummax(CUM.RET)), by = c("variable", "Method")]
...
min_one_month <- high_low_long[, .(MIN.1MONTH = min(RET)),
...
FF_stats <- factors_wide[, {reg <- summary(lm(RET ~ RMRF + SMB + HML + RMW + CMA + MOM,
                                              data = .SD, na.action = na.omit))
  list(ALPHA = reg$coefficients["(Intercept)", "Estimate"],
  T.STAT = reg$coefficients["(Intercept)", "t value"],
  R.SQUARED = reg$r.squared)}, by = c("variable", "Method")]
```
...
turnover_long <- turnover_long[!is.na(Portfolio), .(0.5 * sum(abs(CHANGE))),
                               by = c("Date", "Portfolio", "TYPE")]
turnover_long[, temp := paste(Portfolio, TYPE, sep = " ")]

tikz(file = "Latex/R_graphs/turnover.tex", width = 6, height = 5)
plot8 <- ggplot(turnover_long, aes(x = Date, y = V1)) + geom_bar(stat="identity") +
  labs(y = "Turnover") + facet_wrap(~temp, scales = "free", ncol = 2) +
  theme(axis.title = element_blank(), text = element_text(size = 9))
print(plot8)
dev.off()

ggplot(turnover_long, aes(x = Date, y = V1)) + geom_bar(stat="identity") +
  labs(y = "Turnover") + facet_wrap(~temp, scales = "free", ncol = 2) +
  theme(axis.title = element_blank(), text = element_text(size = 9))

turnover <- turnover_long[, .(TO = mean(V1)), by = c("Portfolio", "TYPE")]
```
...
```{r}
var <- c("RET", independend_variables)
