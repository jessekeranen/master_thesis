...
as.character(expression("FM " ~R^2))
...
turnover_long <- turnover_long[!is.na(Portfolio), .(0.5 * sum(abs(CHANGE))),
                               by = c("Date", "Portfolio", "TYPE")]
turnover_long[, temp := paste("$", substr(Portfolio, 1, 2), "_{",
                              substr(Portfolio, 4, length(Portfolio)), "} \\",
                              TYPE, "$", sep = " ")]
...
```{r}
# TEST!!!
diebold_mariano <- data[, .(FM = mean((EXC.USD.RET - FM.EXP.RET)^2),
                            RF = mean((EXC.USD.RET - RF.EXP.RET)^2),
                            NN = mean((EXC.USD.RET - NN.EXP.RET)^2)), by = Date]

diebold_mariano <- combn(diebold_mariano[!is.na(NN), 2:4], 2, function(x) {
  browser()
  d <- as.matrix(x[, 1] - x[, 2])
  dm <- mean(d) / NeweyWest(lm(d ~ 1), lag = 4)
  return(list(dm, colnames(x)))
})
```
...
```{r}
var <- c("USD.RET", independend_variables)
