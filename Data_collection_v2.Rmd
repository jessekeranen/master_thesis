...
RF_plot <- copy(RF)
RF_plot[, Date := as.Date(paste(year, month, "01", sep = "-"))]

tikz(file = "Latex/R_graphs/rf_ts.tex", width = 5.5, height = 3.5)
plot_rf <- ggplot(RF_plot[Date > as.Date("1989-12-31")], aes(Date, RF)) + geom_line() +
...
Dataset contains companies from multiple countries with different currencies. To be able to pool the data return's and market values have to be comparable between companies from different countries. Daily fx rates are also obtained from Reuters Datastream.
```{r}
...
fx_rates_plot <- copy(fx_rates)
fx_rates_plot[, `:=` (Month = month(Date), Year = year(Date), Day = day(Date))]
fx_rates_plot <- fx_rates_plot[fx_rates_plot[, .I[which.max(Day)],
                                     by = c("Year", "Month", "PCUR")]$V1]


tikz(file = "Latex/R_graphs/fx_rates.tex", width = 5.5, height = 3.5)
plot_fx <- ggplot(fx_rates_plot, aes(Date, FX, color = PCUR)) + geom_line() + scale_y_log10() +
  theme(axis.title = element_blank(), text = element_text(size = 9), 
...
Weekly data is the highest frequency data used in this study. Data from Datastream is downloaded using Excel sheets. To keep size of these files reasonable, data is downloaded in smaller portions. Above introduced function is used to load data from Excel files to R. Only variables used in weekly frequency are the unadjusted price UP and trading volume VO. Weekly data is merged with fx rates so that price can be converted to US dollars. Then returns are calculated both in local currency and US dollars.
```{r, warning=FALSE}
...
tikz(file = "Latex/R_graphs/number_of_companies.tex", width = 5.5, height = 4)
...
Similar to monthly and weekly data I also calculate US dollar values for each of the characteristics used in this study. Nevertheless, if characteristic is used to calculate certain ratio, then the local currency values are always used.
```{r}
...
data[, Q := (WC02999 + DEC.MV - WC02001 - WC03263) / WC02999, by = Company]
...
To avoid results to be driven by the small companies, microcap stocks will be removed from the dataset. Microcap stocks are defined as smallest stocks that account for three percent of month's total market capitalization. Additionally market capitalization of stocks is winsorized between 1% and 99%. In case company's market capitalization in specific month is bigger than 99% of the companies then the market capitalization is replaced by the 99% threshold.
```{r}
...
tikz(file = "Latex/R_graphs/number_of_companies_wo_micro.tex", width = 5.5, height = 4)
